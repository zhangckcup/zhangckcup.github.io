...
>在国内关于房价分析的研究中，使用的模型主要有GM(1,1)模型[1,4,5,6]和多元线性模型[1,12]，其中多元线性模型由使用方式的不同，又有空间异质性定量分析方法——地理加权回归分析 (Geographically weighted regression, GWR)[13]与提取不同特征进行分析的特征模型法（Hedonic model）[8,9]。

如果选取样本的维度较大，在预测完成后尚需使用主成分分析提取主要因素，对样本数据进行降维。若模型中含有时间因素，则可以运用时差相关分析[2]、马尔科夫链预测[3]等方法进行分析。近年来，由于机器学习的兴起，也有部分学者通过BP神经网络[4]与随机森林[5]进行房价预测。
